...
const desc = `We’ve raised $${totalRaised.toLocaleString('en-US')} for ${totalGames.toLocaleString('en-US')}
game${totalGames === 1 ? '' : 's'} so far, and ${unfundedCount.toLocaleString('en-US')}
game${unfundedCount === 1 ? '' : 's'} ${unfundedCount === 1 ? 'remains' : 'remain'} unfunded.`.trim();
...
topEl.textContent = topGame.name; // or include amount: `${topGame.name} — $${topGame.pledged.toLocaleString('en-US')}`
firstGameContainer.appendChild(topEl);

// do the same for the runner up item
const secondEl = document.createElement("p");
secondEl.textContent = secondGame.name; // or `${secondGame.name} — $${secondGame.pledged.toLocaleString('en-US')}`
